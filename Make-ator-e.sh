...
addCompilo ()
{
	...
	then
		...
		echo -e "$CC\t\t=\t#Mettre le compilo ici" >> $1
		echo "" >> $1
	fi
}
...
addSrc ()
{
	...
	echo "" >> $1
	echo "" >> $1
	echo -e "SRCPATH$2\t=\t$paths" >> $1
	echo "" >> $1
	echo -e "SOURCES$2\t=\t\$(addprefix \$(SRCPATH$2)/, \$(SRC$2))" >> $1
	echo "" >> $1
}
...
addHdr ()
{
	...
	echo "" >> $1
	echo "" >> $1
	echo -e "HDRPATH$2\t=\t$pathh" >> $1
	echo "" >> $1
	echo -e "HEADERS$2\t=\t\$(addprefix \$(HDRPATH$2)/, \$(HDR$2))" >> $1
	echo "" >> $1
}
...
basics ()
{
	...
	do
		echo -e "OBJ$count\t\t=\t\$(SRC$count:.c=.o)" >> $1
		echo "" >> $1
		tabobj=( ${tabobj[*]} "\$(OBJ$count)" )

		echo -e "%.o:\t\t\$(SRCPATH$count)/%.c" >> $1
		echo -e "\t\t\t\$(CC) -o \$@ -c \$^ \$(CFLAGS)" >> $1
		echo "" >> $1

		(( count++ ))
	done

    echo -e ".PHONY:\t\tall clean fclean re" >> $1
    echo "" >> $1

    echo -e "all:\t\t\$(NAME)" >> $1
    echo "" >> $1

    echo -e "\$(NAME):\t${tabobj[*]}" >> $1
	if [ $pathl != "" ]
	then
		echo -e "\t\t\t@(cd \$(LIBPATH) && \$(MAKE))" >> $1
		echo -e "\t\t\t\$(CC) -o \$(NAME) ${tabobj[*]} -I \$(LIBHEAD) -L \$(LIBPATH) -l\$(LIBNAME)" >> $1
	else
		echo -e "\t\t\t\$(CC) -o \$(NAME) ${tabobj[*]}" >> $1
	fi
    echo "" >> $1
	...
	if [ $pathl != "" ]
	then
		echo -e "\t\t\t@(cd \$(LIBPATH) && \$(MAKE) clean)" >> $1
	fi
	echo -e "\t\t\trm -rf ${tabobj[*]}" >> $1
    echo "" >> $1

    echo -e "fclean:\t\tclean" >> $1
    echo -e "\t\t\trm -rf \$(NAME)" >> $1
    echo "" >> $1

    echo -e "re:\t\t\tfclean all" >> $1
}
